...
# A dedicated, root-level service to safely sync the bridge password file
  systemd.services.sync-telegram-registration = {
    description = "Sync Telegram Registration to Synapse";
    requiredBy = [ "matrix-synapse.service" ];
    before = [ "matrix-synapse.service" ];
    after = [ "mautrix-telegram.service" ];
    serviceConfig = {
      Type = "oneshot";
      ExecStart = "/bin/sh -c '/run/current-system/sw/bin/cp -f /var/lib/mautrix-telegram/telegram-registration.yaml /var/lib/matrix-synapse/telegram-registration.yaml && /run/current-system/sw/bin/chown matrix-synapse:matrix-synapse /var/lib/matrix-synapse/telegram-registration.yaml'";
